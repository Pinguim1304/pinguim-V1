...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["5511959118827@s.whatsapp.net"] // coloque seu numero
			const mod = [ownerNumber,"5511959118827@s.whatsapp.net"]//seu numero
...
result = fs.readFileSync(`./assets/sorrizin.webp`)
client.sendMessage(from, result, sticker, {quoted: mek })
}
...
case 'help':
				case 'menu':
wew = fs.readFileSync('./assets/pinguimedit2.png')
                client.sendMessage(from, wew, image, {quoted: nay1, caption: help(pushname, prefix) })
				  break
				case 'help1':
				case 'menu1':
...
case 'menuaudio':
		client.sendMessage(from, menuaudio(prefix) , text, {quoted: mek, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "𝕄𝕖𝕟𝕦 𝕡𝕒𝕣𝕒 𝔸𝕦𝕕𝕚𝕠𝕤 🎶", 'jpegThumbnail': fs.readFileSync('kk/sticker/botlogo.webp')}}}})					
                break
case 'logos':
		client.sendMessage(from, logos(prefix, sender), text, {quoted: gay})
                break
case 'imunes':
...
case 'antilink':
					...
					if (Number(args[0]) === 1) {
						...
						client.sendMessage(from,`Atenção  a todos os membros ativos deste grupo o anti-link está ativo. Se você enviar um link de grupo, voce será expulso daqui do grupo`, text)
					} else if (Number(args[0]) === 0) {
...
case 'owner':
                    ...
                    break
case 'ping':
if (isPremium) return reply('Pong!🏓')
else {
reply('Sai random!😑')
}
break
